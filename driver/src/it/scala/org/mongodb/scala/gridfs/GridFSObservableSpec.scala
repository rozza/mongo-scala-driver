...
class GridFSObservableSpec extends RequiresMongoDBISpec with FuturesSpec with BeforeAndAfterEach {
  ...
  "The Scala driver" should "round trip a small file" in {
    ...
    val fileId = gridFSBucket.uploadFromObservable("myFile", Observable(Seq(ByteBuffer.wrap(contentBytes)))).head().futureValue
    ...
    fileInfo.getId should equal(fileId)
  ...
  it should "round trip with small chunks" in {
    ...
    val fileId = gridFSBucket.uploadFromObservable("myFile",
      Observable(Seq(ByteBuffer.wrap(contentBytes), ByteBuffer.wrap(contentBytes))), options).head().futureValue
    filesCollection.countDocuments().head().futureValue should equal(1)
  ...
  it should "round trip with data larger than the internal bufferSize" in {
    ...
    val fileId = gridFSBucket.uploadFromObservable("myFile", Observable(Seq(ByteBuffer.wrap(contentBytes))), options).head().futureValue
    filesCollection.countDocuments().head().futureValue should equal(1)
    chunksCollection.countDocuments().head().futureValue should equal(contentSize / chunkSize)
  ...
  it should "handle custom ids" in {
    ...
    gridFSBucket.uploadFromObservable(fileId, "myFile", Observable(Seq(ByteBuffer.wrap(contentBytes)))).head().futureValue
  ...
  it should "round trip with a bufferSizeBytes of 4096" in {
    ...
    val fileId = gridFSBucket.uploadFromObservable("myFile",
      Observable(Seq(ByteBuffer.wrap(contentBytes))), options).head().futureValue
    filesCollection.countDocuments().head().futureValue should equal(1)
    chunksCollection.countDocuments().head().futureValue should equal(contentSize / chunkSize)
  ...
  it should "create the indexes as expected" in {
    ...
    gridFSBucket.uploadFromObservable("myFile", Observable(Seq(ByteBuffer.wrap(multiChunkString.getBytes())))).futureValue
  ...
  it should "use the user provided codec registries for encoding / decoding data" in {
    val codecRegistry = CodecRegistries.fromRegistries(CodecRegistries.fromCodecs(new UuidCodec(UuidRepresentation.STANDARD)),
    ...
    val fileId = bucket.uploadFromObservable("myFile", Observable(Seq(ByteBuffer.wrap(multiChunkString.getBytes()))),
      new GridFSUploadOptions().metadata(fileMeta)).head().futureValue
  ...
  it should "handle missing file name data when downloading" in {
    val fileId = gridFSBucket.uploadFromObservable("myFile", Observable(Seq(ByteBuffer.wrap(multiChunkString.getBytes())))).head().futureValue
  ...
  it should "cleanup when unsubscribing" in {
    ...
    val contentBytes = new Array[Byte](contentSize)
    scala.util.Random.nextBytes(contentBytes)
    val observer = new Observer[BsonObjectId] {
      var s: Option[Subscription] = None
      def subscription: Subscription = s.get
      override def onSubscribe(subscription: Subscription): Unit =
        s = Some(subscription)

      override def onNext(result: BsonObjectId): Unit = {}

      override def onError(e: Throwable): Unit = {}

      override def onComplete(): Unit = {}
    }
    gridFSBucket.uploadFromObservable("myFile", Observable(List.fill(1024)(ByteBuffer.wrap(contentBytes))))
      .subscribe(observer)
