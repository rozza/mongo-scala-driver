...
class MongoCollectionCaseClassSpec  extends RequiresMongoDBISpec {
  ...
  val codecRegistry = fromRegistries(fromProviders(classOf[User], classOf[Contact], classOf[Optional]), MongoClient.DEFAULT_CODEC_REGISTRY)

  "The Scala driver" should "handle case classes" in withDatabase(databaseName) {
    database =>
      val collection = database.withCodecRegistry(codecRegistry).getCollection[User](collectionName)
  ...
  it should "handle optional values" in withDatabase(databaseName) {
    database =>
      val collection = database.withCodecRegistry(codecRegistry).getCollection[Optional](collectionName)
  ...
  it should "handle converting to case classes where there is extra data" in withDatabase(databaseName) {
    database =>
      val collection = database.withCodecRegistry(codecRegistry).getCollection[Contact](collectionName)
