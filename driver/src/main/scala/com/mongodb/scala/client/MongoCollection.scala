...
import com.mongodb.scala.core.Helpers.{ DefaultsTo, classTagToClassOf }
import com.mongodb.scala.client.ObservableHelper.{ observe, observeAndFlatten, observeCompleted, observeLong }
...
case class MongoCollection[T](private val wrapped: JMongoCollection[T]) {
   ...
   */
  def aggregate[C](pipeline: List[Bson])(implicit e: C DefaultsTo Document, ct: ClassTag[C]): AggregateObservable[C] =
   ...
   */
  def bulkWrite(requests: List[_ <: WriteModel[_ <: T]]): Observable[BulkWriteResult] =
    observe(wrapped.bulkWrite(requests.asJava.asInstanceOf[util.List[_ <: WriteModel[_ <: T]]], _: SingleResultCallback[BulkWriteResult]))
   ...
   * @return a Observable with a single element the BulkWriteResult
   */
  def bulkWrite(requests: List[_ <: WriteModel[_ <: T]], options: BulkWriteOptions): Observable[BulkWriteResult] =
    observe(wrapped.bulkWrite(
      requests.asJava.asInstanceOf[util.List[_ <: WriteModel[_ <: T]]],
      options,
   ...
   */
  def insertOne(document: T): Observable[Completed] = observeCompleted(wrapped.insertOne(document, _: SingleResultCallback[Void]))
   ...
   *         com.mongodb.DuplicateKeyException or com.mongodb.MongoException
   */
  def insertMany(documents: List[_ <: T]): Observable[Completed] =
   ...
   *         com.mongodb.DuplicateKeyException or com.mongodb.MongoException
   */
  def insertMany(documents: List[_ <: T], options: InsertManyOptions): Observable[Completed] =
   ...
   */
  def replaceOne(filter: Bson, replacement: T): Observable[UpdateResult] =
   ...
   * @return a Observable with a single element the UpdateResult
   */
  def replaceOne(filter: Bson, replacement: T, options: UpdateOptions): Observable[UpdateResult] =
   ...
   *         returned
   */
  def findOneAndDelete(filter: Bson): Observable[T] = observe(wrapped.findOneAndDelete(filter, _: SingleResultCallback[T]))
   ...
   *         returned
   */
  def findOneAndDelete(filter: Bson, options: FindOneAndDeleteOptions): Observable[T] =
    observe(wrapped.findOneAndDelete(filter, options, _: SingleResultCallback[T]))
   ...
   */
  def findOneAndReplace(filter: Bson, replacement: T): Observable[T] =
    observe(wrapped.findOneAndReplace(filter, replacement, _: SingleResultCallback[T]))
   ...
   *         query filter, then null will be returned
   */
  def findOneAndReplace(filter: Bson, replacement: T, options: FindOneAndReplaceOptions): Observable[T] =
    observe(wrapped.findOneAndReplace(filter, replacement, options, _: SingleResultCallback[T]))
   ...
   *         query filter, then null will be returned
   */
  def findOneAndUpdate(filter: Bson, update: Bson): Observable[T] =
    observe(wrapped.findOneAndUpdate(filter, update, _: SingleResultCallback[T]))
   ...
   *         query filter, then null will be returned
   */
  def findOneAndUpdate(filter: Bson, update: Bson, options: FindOneAndUpdateOptions): Observable[T] =
    observe(wrapped.findOneAndUpdate(filter, update, options, _: SingleResultCallback[T]))
