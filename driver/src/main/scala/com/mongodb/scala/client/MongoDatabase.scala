...
import com.mongodb.scala.core.Helpers.DefaultsTo
import com.mongodb.scala.client.ObservableHelper.{ observe, observeCompleted }
...
case class MongoDatabase(private val wrapped: JMongoDatabase) {
   ...
   */
  def runCommand[T](command: Bson)(implicit e: T DefaultsTo Document, ct: ClassTag[T]): Observable[T] =
    observe(wrapped.runCommand[T](command, ct.runtimeClass.asInstanceOf[Class[T]], _: SingleResultCallback[T]))
   ...
   * @return a Observable containing the command result
   */
  def runCommand[T](command: Bson, readPreference: ReadPreference)(implicit e: T DefaultsTo Document, ct: ClassTag[T]): Observable[T] =
    observe(wrapped.runCommand(command, readPreference, ct.runtimeClass.asInstanceOf[Class[T]], _: SingleResultCallback[T]))
   ...
   */
  def listCollections[T]()(implicit e: T DefaultsTo Document, ct: ClassTag[T]): ListCollectionsObservable[T] =
    ListCollectionsObservable(wrapped.listCollections(ct.runtimeClass.asInstanceOf[Class[T]]))
