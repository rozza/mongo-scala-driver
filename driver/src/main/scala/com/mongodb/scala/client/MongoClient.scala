...
import com.mongodb.async.client.{ MongoClient => JMongoClient, MongoClientSettings, MongoClients }

import com.mongodb.scala.core.Helpers.DefaultsTo
import com.mongodb.scala.core.codecs.DocumentCodecProvider
...
case class MongoClient(private val wrapped: JMongoClient) extends Closeable {
   ...
   */
  def listDatabases[T]()(implicit e: T DefaultsTo Document, ct: ClassTag[T]): ListDatabasesObservable[T] =
    ListDatabasesObservable(wrapped.listDatabases(ct.runtimeClass.asInstanceOf[Class[T]]))
}
