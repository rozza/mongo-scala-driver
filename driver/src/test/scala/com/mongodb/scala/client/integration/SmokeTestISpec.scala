...
class SmokeTestISpec extends RequiresMongoDBISpec {
  ...
  "The Scala driver" should "handle common scenarios without error" in withDatabase(databaseName) {
    database =>
      ...
      database.createCollection(collectionName).futureValue.head shouldBe Completed()
      ...
      collection.insertOne(document).futureValue.head shouldBe Completed()
      ...
      info("drop the index")
      collection.dropIndex("test_1").futureValue.head shouldBe Completed()
      ...
      collection.renameCollection(new MongoNamespace(databaseName, newCollectionName)).futureValue.head shouldBe Completed()
      ...
      newCollection.drop().futureValue.head shouldBe Completed()
