...
import com.mongodb.scala.core.collection.immutable.{Document => ImmutableDocument}
...
trait IterableImplicits {
  ...
  implicit def bsonArrayToIterableDocument(value: BsonArray): Iterable[ImmutableDocument] = value.asScala map (v => ImmutableDocument(v.asDocument()))
