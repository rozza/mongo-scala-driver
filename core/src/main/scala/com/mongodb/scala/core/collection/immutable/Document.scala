...
package com.mongodb.scala.core.collection.immutable
...
import com.mongodb.scala.core.collection
 ...
 * is no variance in the value type and it always has to be a `BsonValue`.  The [[com.mongodb.scala.core.client.Implicits]]
 ...
 *       the underlying [[com.mongodb.scala.core.client.codecs.ImmutableDocumentCodec]] codec on insertion to the database.
 * @param underlying the underlying BsonDocument which stores the data.
 */
case class Document(protected[core] val underlying: BsonDocument)
   ...
   * @return a new document
   */
  protected[core] def apply(underlying: BsonDocument) = new Document(underlying)
