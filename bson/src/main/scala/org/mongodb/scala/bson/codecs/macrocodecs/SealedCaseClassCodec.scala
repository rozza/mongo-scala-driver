...
private[codecs] object SealedCaseClassCodec {
  ...
  def createCodec[T: c.WeakTypeTag](c: whitebox.Context)(codecRegistry: c.Expr[CodecRegistry]): c.Expr[Codec[T]] = {
    ...
    def allSubclasses(s: Symbol): Set[Symbol] = {
      ...
      directSubClasses ++ directSubClasses.flatMap({ case s: Symbol => allSubclasses(s) })
    }
    val subClasses: List[Type] = allSubclasses(mainType.typeSymbol).map(_.asClass).map(_.toType).filter(isCaseClass).toList
