...
private[codecs] object SealedCaseClassCodec {
  ...
  def createCodec[T: c.WeakTypeTag](c: whitebox.Context)(codecRegistry: c.Expr[CodecRegistry]): c.Expr[Codec[T]] = {
    ...
    if (subClasses.isEmpty) c.abort(c.enclosingPosition, "No known subclasses of the sealed class")
    ...
    c.Expr[Codec[T]](
      q"""
        ...
        import org.mongodb.scala.bson.codecs.macrocodecs.MacroSealedCodecHelper

        case class $codecName(codecRegistry: CodecRegistry) extends MacroSealedCodecHelper[$classTypeName] {
          val subClassesMap = $subClassesMap
          val subClassesMapInv = subClassesMap.map(_.swap)
          val clazzToCaseClassMap = $clazzToCaseClassMap
          ...
          def writeCaseClassData(className: String, writer: BsonWriter, value: $mainType, encoderContext: EncoderContext, bsonNull: BsonValue) = $writeValue
        }
